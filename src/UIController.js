export function initUI() {
  ...
  toastContainer.className =
    "fixed bottom-4 left-1/2 -translate-x-1/2 z-[200] flex flex-col gap-2";
  document.body.appendChild(toastContainer);

  window.showToast = function (msg) {
    const t = document.createElement("div");
    t.className =
      "bg-surface-container-highest border border-primary/50 text-primary text-xs px-4 py-2 rounded-full shadow-2xl backdrop-blur transition-all duration-300 opacity-0 translate-y-4";
  ...
  document
    .querySelectorAll('button:not([id]), a[href="#"]:not(.nav-item)')
    .forEach((el) => {
      if (
        !el.classList.contains("layer-btn") &&
        !el.classList.contains("quick-prompt")
      ) {
  ...
  if (emBtn) {
    emBtn.addEventListener("click", () => {
      ...
      overlay.className =
        "fixed inset-0 z-[150] border-[8px] border-error pointer-events-none animate-pulse";

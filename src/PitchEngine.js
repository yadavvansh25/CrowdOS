export function PitchEngine() {
  ...
  const PX1 = 56, PX2 = 644, PY1 = 46, PY2 = 404;
  ...
  const TEAM_A_COLOR = '#adc6ff', TEAM_A_FILTER = 'glow-blue';
  const TEAM_B_COLOR = '#ffb786', TEAM_B_FILTER = 'glow-amber';
  ...
  const playerDefs = [
    ...
    { team:'A', num:'3',  speed:.35,  pos:[{x:120,y:155},{x:130,y:185},{x:115,y:215},{x:120,y:155}] },
    { team:'A', num:'5',  speed:.38,  pos:[{x:125,y:295},{x:140,y:265},{x:130,y:245},{x:125,y:295}] },
    { team:'A', num:'4',  speed:.36,  pos:[{x:170,y:200},{x:185,y:230},{x:175,y:260},{x:170,y:200}] },
    { team:'A', num:'8',  speed:.42,  pos:[{x:240,y:175},{x:265,y:195},{x:250,y:225},{x:240,y:175}] },
    { team:'A', num:'6',  speed:.40,  pos:[{x:245,y:270},{x:265,y:250},{x:255,y:280},{x:245,y:270}] },
    { team:'A', num:'10', speed:.55,  pos:[{x:310,y:195},{x:335,y:210},{x:320,y:240},{x:295,y:220},{x:310,y:195}] },
    { team:'A', num:'11', speed:.52,  pos:[{x:300,y:265},{x:285,y:245},{x:315,y:255},{x:300,y:265}] },
    // Team B — right side
    { team:'B', num:'1',  speed:0,    gk:true,  pos:[{x:632,y:225}] },
    { team:'B', num:'2',  speed:.35,  pos:[{x:580,y:155},{x:565,y:185},{x:575,y:215},{x:580,y:155}] },
    { team:'B', num:'5',  speed:.38,  pos:[{x:575,y:295},{x:560,y:265},{x:570,y:245},{x:575,y:295}] },
    { team:'B', num:'4',  speed:.36,  pos:[{x:530,y:200},{x:515,y:230},{x:525,y:260},{x:530,y:200}] },
    { team:'B', num:'8',  speed:.42,  pos:[{x:460,y:175},{x:435,y:195},{x:450,y:225},{x:460,y:175}] },
    { team:'B', num:'6',  speed:.40,  pos:[{x:455,y:270},{x:435,y:250},{x:445,y:280},{x:455,y:270}] },
    { team:'B', num:'10', speed:.55,  pos:[{x:390,y:195},{x:365,y:210},{x:380,y:240},{x:405,y:220},{x:390,y:195}] },
    { team:'B', num:'7',  speed:.52,  pos:[{x:400,y:265},{x:415,y:245},{x:385,y:255},{x:400,y:265}] },
  ];

  // Runtime state per player
  const players = playerDefs.map((d,i) => ({
    ...d,
    x: d.pos[0].x, y: d.pos[0].y,
    tx: d.pos[0].x, ty: d.pos[0].y,
    wpIdx: 0,
    wpT: 0,
    hasBall: false,
    id: 'player-' + i,
  ...
  players.forEach(p => {
    ...
    // Shadow ellipse
    const sh = el('ellipse', { cx: p.x, cy: p.y+7, rx:6, ry:2.5, fill:'rgba(0,0,0,.35)', id: p.shadowId });
    ...
    if(!p.gk) {
      ...
      g.appendChild(el('circle', { cx:0, cy:0, r:7, fill: color + '22', stroke: color, 'stroke-width':'1.2', filter:`url(#${filter})` }));
      // Inner fill
      g.appendChild(el('circle', { cx:0, cy:0, r:4.5, fill: color, filter:`url(#${filter})` }));
    } else {
      const gkColor = p.team === 'A' ? '#a5f3fc' : '#fbbf24';
      g.appendChild(el('circle', { cx:0, cy:0, r:8, fill: gkColor+'33', stroke: gkColor, 'stroke-width':'1.5', filter:`url(#${filter})` }));
      g.appendChild(el('circle', { cx:0, cy:0, r:5.5, fill: gkColor, filter:`url(#${filter})` }));
    }
    // Jersey number
    const txt = el('text', { x:0, y:4, 'text-anchor':'middle', 'font-size':'5.5', 'font-weight':'800',
      'font-family':'JetBrains Mono,monospace', fill: p.team==='A'?'#002e6a':'#502400', 'pointer-events':'none' });
  ...
  // Ball DOM element
  const ballOuter = el('circle', { cx:0, cy:0, r:7, fill:'url(#ball-glow)', filter:'url(#glow-white)', opacity:'.95', id:'ball-outer' });
  ...
  [0,72,144,216,288].forEach(a => {
    ...
    const line = el('line', { x1:0, y1:0,
      x2: Math.cos(rad)*4, y2: Math.sin(rad)*4,
      stroke:'rgba(0,0,0,.25)', 'stroke-width':'.8' });
  ...
  function shootToGoal(ownerIdx) {
    ...
    const spd = 6.5;
    ballVx = dx/dist*spd; ballVy = dy/dist*spd;
    ballMoving = true; ballOwner = null; ballTargetPlayer = -1; // heading to goal
  ...
  function triggerGoal(scoringTeam) {
    ...
    if(!sa || !sb) return;
    if(scoringTeam === 'A') sa.textContent = String(Number(sa.textContent)+1);
    else sb.textContent = String(Number(sb.textContent)+1);
    // Goal flash
    const fl = document.getElementById('goal-flash');
    if(fl) { fl.classList.remove('firing'); void fl.offsetWidth; fl.classList.add('firing'); }
    // Event badge
    spawnBadge('⚽ GOAL!');
  ...
  function drawTrail(x1,y1,x2,y2,color) {
    ...
    const anim = el('animate', {
      attributeName:'stroke-dashoffset', from:'180', to:'0',
      dur:'.4s', fill:'freeze'
    });
    const fade = el('animate', {
      attributeName:'opacity', from:'.8', to:'0',
      dur:'1s', begin:'.4s', fill:'freeze'
    });
  ...
  function updatePlayers() {
    ...
    // GK drift
    players.filter(p=>p.gk).forEach(p => {
    ...
    if(ballMoving) {
      ...
      if(passTimer <= 0) {
        ...
        if(goalShotTimer <= 0 && Math.random() < 0.2) {
          ...
          const color = players[ballOwner].team==='A'?TEAM_A_COLOR:TEAM_B_COLOR;
  ...
  function render() {
    ...
    players.forEach(p => {
      ...
      if(sh) { sh.setAttribute('cx', p.x.toFixed(1)); sh.setAttribute('cy', (p.y+7).toFixed(1)); }
    });
    // Ball
    ballG2.setAttribute('transform', `translate(${ball.x.toFixed(1)},${ball.y.toFixed(1)}) rotate(${Date.now()*0.15})`);
  }
  ...
  function buildCrowdWave() {
    ...
    const segW = 22, count = Math.floor(590/segW);
    for(let i=0;i<count;i++) {
      const nx = 55 + i*segW;
      const rn = el('rect', { x:nx, y:25, width:segW-1, height:20, rx:1, fill:'rgba(255,200,120,1)' });
      const anim = el('animate', {
      ...
      const rs = el('rect', { x:nx, y:405, width:segW-1, height:20, rx:1, fill:'rgba(255,200,120,1)' });
  ...
  setTimeout(()=>{ if(ballOwner!==null) { shootToGoal(ballOwner); } }, 8000);



// ── Incident checkboxes
document.querySelectorAll('#tab-incident input[type="checkbox"]').forEach(cb => {

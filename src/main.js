...
function tick() {
  ...
  const s =
    ...
    ":" +
    p(n.getUTCSeconds());
  ["utc-clock", "footer-clock"].forEach((id) => {
    const e = document.getElementById(id);
    if (e) e.textContent = id === "footer-clock" ? "UTC: " + s : s;
  });
}
tick();
...
const feedItems = [
  {
    ...
    time: "14:32",
    desc: "Section 114, Row J. Dispatching Unit Med-2.",
  },
  {
    icon: "security",
    color: "tertiary",
    ...
    time: "14:28",
    desc: "Perimeter Gate 3. Secondary fence breach alarm.",
  },
  {
    ...
    time: "14:25",
    desc: "Liquid spill in Suite Corridor B.",
  },
  {
    icon: "group",
    ...
    time: "14:15",
    desc: "Logistics Team Alpha out. Beta active.",
  },
  {
    icon: "bolt",
    color: "tertiary",
    ...
    time: "14:10",
    desc: "Solar panel efficiency 91.2%. Nominal.",
  },
  {
    icon: "directions",
    color: "primary",
    ...
    time: "14:05",
    desc: "Gate 7 overflow via North Concourse.",
  },
];
...
function getResp(input) {
  ...
  for (const k of Object.keys(RESP)) {
    if (k !== "default" && l.includes(k))
      return { text: RESP[k], cached: CACHED.has(k) };
  }
  if (/medical|hurt|help|assist|ambulan/.test(l))
    return { text: RESP.medical, cached: false };
...
function sendMsg(text) {
  ...
  setTimeout(() => {
    ...
    document.getElementById("session-stats").textContent =
      `${queryCount} ${queryCount === 1 ? "query" : "queries"} · ${cacheHits} cache hits`;
    appendMsg(
      "ai",
      resp,
      `AI · ${cached ? "⚡ Redis cached · " : ""}${Math.round(delay)}ms · ${langName}`,
    );
  }, delay);
}
...
document.querySelectorAll(".quick-prompt").forEach((b) => {
  b.addEventListener("click", () =>
    sendMsg(b.textContent.trim().replace(/^[^\s]+ /, "")),
  );
});
...
document.getElementById("stt-btn").addEventListener("click", () => {
  if (!("webkitSpeechRecognition" in window || "SpeechRecognition" in window)) {
    appendMsg("ai", "⚠️ Voice input not supported in this browser.", "System");
    return;
  }
  ...
  r.onstart = () => {
    mi.textContent = "mic_off";
    mi.classList.add("text-error");
  };
  r.onresult = (e) => {
  ...
  r.onend = () => {
    mi.textContent = "mic";
    mi.classList.remove("text-error");
  };
  r.start();
});
document.getElementById("tts-btn").addEventListener("click", () => {
  ...
  const last = Array.from(msgs)
    .reverse()
    .find((m) => m.querySelector(".text-primary"));
...
document.getElementById("lang-select").addEventListener("change", (e) => {
  document.getElementById("current-lang").textContent =
    LANG_NAMES[e.target.value] || e.target.value;
});

// Accessibility toggles
document.getElementById("hc-toggle").addEventListener("change", (e) => {
  document.body.style.filter = e.target.checked ? "contrast(1.35)" : "";
});
document.getElementById("lt-toggle").addEventListener("change", (e) => {
  document.documentElement.style.fontSize = e.target.checked ? "18px" : "";
});
document.getElementById("rm-toggle").addEventListener("change", (e) => {
  document.documentElement.style.setProperty("--motion", "none");
  document.body.style.animationDuration = e.target.checked ? "0s" : "";
});
...
document.getElementById("alert-gate7")?.addEventListener("mouseenter", () => {
  document.getElementById("tip-gate7").classList.remove("hidden");
});
document.getElementById("alert-gate7")?.addEventListener("mouseleave", () => {
  document.getElementById("tip-gate7").classList.add("hidden");
});
document.getElementById("alert-med")?.addEventListener("mouseenter", () => {
  document.getElementById("tip-med").classList.remove("hidden");
});
document.getElementById("alert-med")?.addEventListener("mouseleave", () => {
  document.getElementById("tip-med").classList.add("hidden");
});
...
setInterval(() => {
  ...
  matchSecs++;
  const m = Math.floor(matchSecs / 60),
    s = matchSecs % 60;
